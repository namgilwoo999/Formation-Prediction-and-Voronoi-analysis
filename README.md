...
FOOTBALL_ANALYSIS/
├── main.py                          # 메인 실행 파일
├── improved_voronoi_analysis.py     # Voronoi 분석 및 통계
├── interactive_preview.py           # 구간 미리보기 도구
│
├── annotation/                      # 영상 주석 모듈
│   ├── football_video_processor.py # 비디오 처리 파이프라인
│   ├── object_annotator.py         # 객체 주석 (선수, 공, 심판)
│   ├── keypoints_annotator.py      # 키포인트 주석
│   └── ...
│
├── tracking/                        # 객체 추적 모듈
│   ├── tracker.py                   # 메인 추적 클래스 (ByteTrack)
│   ├── keypoints_tracker.py        # 경기장 키포인트 추적
│   └── team_consistency_checker.py # 팀 할당 일관성 검증
│
├── team_assigner/                   # 팀 색상 자동 할당
│   └── team_assigner.py             # KMeans 기반 유니폼 색상 분류
│
├── position_mappers/                # 좌표 변환 모듈
│   ├── homography.py                # 호모그래피 행렬 계산
│   └── object_position_mapper.py   # 이미지 좌표 → 실제 구장 좌표
│
├── formation_analysis/              # 포메이션 분석 모듈
│   ├── classify_formation.py       # 포메이션 분류 (Hungarian 알고리즘)
│   ├── formation_clustering.py     # 포메이션 클러스터링
│   ├── formation_timeline_visualizer.py  # 타임라인 시각화
│   ├── formation_distribution_plotter.py # 분포 차트
│   └── formation_confusion_matrix.py     # 혼동 행렬 분석
│
├── ball_to_player_assignment/       # 볼 소유 판정
│   └── player_ball_assigner.py      # 선수-공 거리 기반 할당
│
├── file_writing/                    # 결과 저장 모듈
│   └── tracks_json_writer.py        # JSON 형식으로 추적 데이터 저장
│
├── utils/                           # 유틸리티 함수
│   ├── video_utils.py               # 비디오 읽기/쓰기
│   ├── bbox_utils.py                # 바운딩 박스 유틸
│   ├── formation_utils.py           # 포메이션 유틸
│   └── ...
│
├── models/                          # 모델 가중치 (사용자가 배치)
│   ├── best.pt                      # YOLOv11 객체 탐지 모델
│   └── keypoints_best.pt            # 키포인트 탐지 모델
│
├── input_videos/                    # 입력 영상 폴더
├── output_videos/                   # 출력 영상 폴더
└── stubs/                           # 중간 결과 저장 폴더
```
